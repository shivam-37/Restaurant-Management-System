                ...
                <>
                    ...
                    <div className="fixed inset-0 flex items-center justify-center z-50 p-4 pointer-events-none">
                        <motion.div
                            ...
                            className="theme-card-item rounded-3xl p-8 max-w-md w-full shadow-2xl pointer-events-auto relative border border-orange-500/20"
                            style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}
                        >
                            {/* Close Button */}
                            <button
                                onClick={onClose}
                                className="absolute top-5 right-5 w-8 h-8 rounded-full flex items-center justify-center border border-black/10 dark:border-white/10 text-gray-400 hover:text-orange-500 transition cursor-pointer"
                            >
                                <X className="w-4 h-4" />
                            </button>
                            ...
                            <div className="text-center mb-6">
                                <div className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-4 shadow-lg ${isIndigo ? 'bg-[#f97316]/10 border-[#f97316]/20 shadow-orange-500/10' : 'bg-orange-500/10 border-orange-500/20 shadow-orange-500/10'} border`}>
                                    <ChefHat className={`w-8 h-8 ${isIndigo ? 'text-[#f97316]' : 'text-orange-500'}`} />
                                </div>
                                ...
                                <p className="text-xs text-gray-700 dark:text-gray-400 max-w-xs mx-auto">
                                    ...
                                    <br />
                                    <span className={`font-bold tracking-wider mt-1 inline-block ${isIndigo ? 'text-[#f97316]' : 'text-orange-500'}`}>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <button
                                    ...
                                    className={`w-full py-4 text-white rounded-2xl text-sm font-bold shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer ${isIndigo ? 'bg-[#f97316] shadow-orange-500/25 hover:bg-indigo-700' : 'bg-orange-500 shadow-orange-500/25 hover:bg-orange-600'}`}
                                >
                                    {isLoading ? (
                                ...
                                <div className="pt-1 flex flex-col items-center justify-center gap-2">
                                    {timeLeft > 0 ? (
                                        <div className={`flex items-center gap-2 text-xs text-gray-700 dark:text-gray-400 px-4 py-2 rounded-xl border ${isIndigo ? 'bg-[#f97316]/5 border-[#f97316]/10' : 'bg-orange-500/5 border-orange-500/10'}`}>
                                            <Clock className={`w-4 h-4 animate-pulse ${isIndigo ? 'text-[#f97316]' : 'text-orange-500'}`} />
                                            <span>Resend code in</span>
                                            <span className={`font-mono font-bold ${isIndigo ? 'text-[#f97316]' : 'text-orange-500'}`}>{formatTime(timeLeft)}</span>
                                        </div>
                                    ) : (
                                        <button
                                            type="button"
