    ...
    return (
        <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
            ...
            <div className="fixed inset-0 overflow-hidden -z-10">
                ...
                <div className="absolute top-0 -right-40 w-[500px] h-[500px] bg-amber-600 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob animation-delay-2000"></div>
            ...
            >
                <div className="p-6 border-b border-gray-800/50">
                    <motion.div whileHover={{ scale: 1.05 }} className="flex items-center space-x-3">
                        <div className="relative">
                            <div className="absolute inset-0 bg-gradient-to-r from-amber-500 to-purple-500 rounded-xl blur-lg opacity-50"></div>
                            <div className="relative w-10 h-10 bg-gradient-to-br from-amber-600 to-purple-600 rounded-xl flex items-center justify-center">
                                <SparklesIcon className="w-5 h-5 text-white" />
                            </div>
                        </div>
                        <span className="text-xl font-bold bg-gradient-to-r from-amber-400 to-purple-400 bg-clip-text text-transparent">RestoAdmin</span>
                    </motion.div>
                </div>
                    ...
                    {navItems.map((item, index) => {
                        ...
                        return (
                            <motion.button
                                ...
                                className={`relative w-full flex items-center px-4 py-3 rounded-xl transition-all duration-200 group overflow-hidden ${activeTab === item.name ? 'text-amber-600 dark:text-white' : 'opacity-50 hover:opacity-100'
                                    }`}
                            >
                                {activeTab === item.name && (
                                ...
                                <Icon className={`h-6 w-6 mr-3 relative z-10 transition-colors ${activeTab === item.name ? 'text-amber-500' : 'opacity-60 group-hover:opacity-100'}`} />
                ...
                <div className="p-4 border-t border-black/5 dark:border-white/5">
                    <div className="relative">
                        ...
                        >
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-600 to-purple-600 flex items-center justify-center text-white font-bold shadow-lg shadow-amber-500/20">{user?.name?.charAt(0) || 'A'}</div>
                            <div className="flex-1 min-w-0 text-left">
                                <p className="text-sm font-bold truncate">{user?.name}</p>
                                <p className="text-xs opacity-50 truncate">Admin Console</p>
                            </div>
            ...
            <main className={`flex-1 md:ml-72 relative overflow-y-auto`}>
                <motion.header initial={{ y: -100 }} animate={{ y: 0 }} className="sticky top-0 z-10 backdrop-blur-xl border-b" style={{ background: 'var(--navbar-bg)', borderColor: 'var(--border-color)' }}>
                    <div className="flex items-center justify-between px-8 py-4">
                        <div>
                            ...
                            <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>{selectedRestaurant ? selectedRestaurant.name : 'Platform Overview'}</p>
                        </div>
                                ...
                                <div className="space-y-8">
                                    ...
                                    {selectedRestaurant && (
                                        <motion.div
                                            initial={{ opacity: 0, y: -20 }}
                                            animate={{ opacity: 1, y: 0 }}
                                            className="theme-card bg-amber-600/5 border border-amber-500/20 rounded-3xl p-6 flex flex-col md:flex-row items-center gap-6"
                                        >
                                            <div className="w-20 h-20 bg-amber-600 rounded-2xl flex items-center justify-center text-3xl font-bold shadow-lg shadow-amber-600/20 text-white">
                                            ...
                                            <div className="flex-1 text-center md:text-left">
                                                <div className="flex items-center justify-center md:justify-start gap-3 mb-1">
                                                    ...
                                                    <span className="px-2 py-0.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded-full text-[10px] font-black uppercase tracking-widest border border-amber-500/20">Active Establishment</span>
                                                </div>
                                                <p className="opacity-60 text-sm max-w-xl font-medium">{selectedRestaurant.description || "No description provided for this establishment."}</p>
                                                <div className="flex items-center justify-center md:justify-start gap-4 mt-3">
                                                    <div className="flex items-center gap-1 text-xs text-amber-500 font-bold uppercase tracking-wide">
                                            ...
                                            <button
                                                ...
                                                className="px-6 py-3 bg-amber-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-amber-600/30 hover:bg-amber-500 transition active:scale-95"
                                            >
                                                Platform View
                                            </button>
                                        </motion.div>
                                    ...
                                    <motion.div variants={staggerContainer} initial="initial" animate="animate" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                        {stats.map((stat, idx) => {
                                            ...
                                            return (
                                                <motion.div key={idx} variants={fadeInUp} whileHover={{ y: -4 }} className="group relative theme-card rounded-2xl p-6 hover:border-amber-500/30 transition-all duration-300 overflow-hidden">
                                                    ...
                                                    <div className="flex items-start justify-between relative z-10">
                                                        ...
                                                        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${stat.color} p-3 shadow-lg shadow-amber-500/20 flex items-center justify-center`}><Icon className="w-6 h-6 text-white" /></div>
                                                    </div>
                                                </motion.div>
                                            );
                                        })}
                                    ...
                                    <motion.div variants={fadeInUp} initial="initial" animate="animate" className="theme-card rounded-[2.5rem] p-8 border border-black/5 shadow-2xl relative overflow-hidden">
                                        ...
                                        <div className="flex items-center justify-between mb-8 relative z-10">
                                            <div className="flex items-center gap-4">
                                                <div className="w-14 h-14 bg-gradient-to-br from-amber-600 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg shadow-amber-600/20 text-white">
                                                    <SparklesIcon className="w-7 h-7" />
                                                </div>
                                                <div>
                                                    <h2 className="text-2xl font-black uppercase tracking-tighter">Inventory Guard AI</h2>
                                                    <p className="text-xs font-bold opacity-40 uppercase tracking-widest">NVIDIA NIM Predictive Intelligence</p>
                                                </div>
                                            </div>
                                            <button
                                                onClick={fetchAIPredictions}
                                                disabled={isPredicting}
                                                className="p-3 theme-card-item rounded-xl border border-black/5 hover:border-amber-500 transition group"
                                            >
                                                <ArrowPathIcon className={`w-5 h-5 text-amber-500 group-hover:rotate-180 transition-transform duration-500 ${isPredicting ? 'animate-spin' : ''}`} />
                                            </button>
                                        </div>
                                        ...
                                        {isPredicting ? (
                                            <div className="space-y-4 relative z-10">
                                                {[1, 2].map(i => (
                                                    <div key={i} className="h-20 bg-amber-500/5 rounded-2xl animate-pulse"></div>
                                                ))}
                                            </div>
                                        ) : predictions.length > 0 ? (
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
                                                {predictions.map((p, idx) => (
                                                    <div key={idx} className="theme-card-item border border-black/5 p-6 rounded-2xl group hover:border-amber-500/30 transition-all shadow-sm">
                                                        <div className="flex items-start justify-between mb-3">
                                                            <h4 className="font-bold">{p.name}</h4>
                                                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${p.risk === 'High' ? 'bg-red-500/10 text-red-500 border border-red-500/20' : 'bg-amber-500/10 text-amber-500 border border-amber-500/20'
                                                                }`}>
                                                                {p.risk} Risk
                                                            </span>
                                                        </div>
                                                        <p className="text-xs opacity-60 mb-4 line-clamp-2 leading-relaxed font-medium">{p.reason}</p>
                                                        <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-amber-600 dark:text-amber-400 bg-amber-500/5 p-2 rounded-lg">
                                    ...
                                    {!selectedRestaurant && analytics.restaurantStats && analytics.restaurantStats.length > 0 && (
                                        <motion.div variants={fadeInUp} className="theme-card rounded-[2.5rem] border border-black/5 shadow-2xl overflow-hidden">
                                            <div className="p-8 border-b border-black/5 flex items-center justify-between bg-amber-500/5">
                                                <div>
                                                    <h2 className="text-2xl font-black uppercase tracking-tighter">Establishment Performance</h2>
                                                    <p className="text-xs font-bold opacity-40 uppercase tracking-widest mt-1">Cross-restaurant analytics comparison</p>
                                                </div>
                                                <button
                                                    onClick={() => setActiveTab('Restaurants')}
                                                    className="px-6 py-3 bg-amber-600 text-white rounded-xl hover:bg-amber-700 shadow-lg shadow-amber-600/20 transition font-black text-[10px] uppercase tracking-widest"
                                                >
                                                    Management Hub
                                                </button>
                                            </div>
                                                ...
                                                <table className="w-full text-left">
                                                    <thead>
                                                        <tr className="bg-black/5 dark:bg-white/5 text-[10px] font-black uppercase tracking-[0.2em] opacity-40">
                                                            ...
                                                            <th className="px-8 py-5 text-center">Hub</th>
                                                        </tr>
                                                    </thead>
                                                        ...
                                                        {analytics.restaurantStats.map((stat) => (
                                                            <tr key={stat.id} className="group hover:bg-black/5 transition-all">
                                                                <td className="px-8 py-6">
                                                                    <div className="flex items-center gap-4">
                                                                        <div className="w-12 h-12 bg-gradient-to-br from-amber-500 to-purple-600 rounded-2xl flex items-center justify-center text-white font-black text-lg shadow-lg shadow-amber-500/20">
                                                                            {stat.name.charAt(0)}
                                                                        </div>
                                                                        <div>
                                                                            <p className="text-sm font-black uppercase tracking-wider">{stat.name}</p>
                                                                            <p className="text-[10px] font-bold text-amber-500 uppercase tracking-widest">{stat.cuisine} Cuisine</p>
                                                                        </div>
                                                                    </div>
                                                                </td>
                                                                <td className="px-8 py-6">
                                                                ...
                                                                <td className="px-8 py-6 text-center">
                                                                    <button
                                                                        ...
                                                                        className="p-3 opacity-20 group-hover:opacity-100 theme-card-item border border-black/5 hover:border-amber-500 hover:text-amber-500 rounded-xl transition-all"
                                                                    >
