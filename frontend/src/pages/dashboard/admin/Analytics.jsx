...
const Analytics = () => {
    ...
    if (loading) return (
        <div className="space-y-8 animate-pulse">
            <div className="h-10 bg-amber-500/10 rounded-2xl w-1/3" />
    ...
    if (!data) return (
        <div className="text-center py-20 theme-card rounded-3xl border border-black/5 border-dashed">
            ...
            <p className="opacity-40 font-black uppercase tracking-widest text-xs">Awaiting data stream...</p>
        </div>
    );
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black uppercase tracking-tighter">Business Logic</h1>
                    <p className="opacity-40 text-[10px] font-black uppercase tracking-[0.2em] mt-1">{selectedRestaurant?.name || 'Platform-Wide Intelligence'}</p>
                </div>
                <button
                    onClick={fetchAnalytics}
                    className="flex items-center gap-3 px-6 py-3 bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-amber-600 hover:text-white transition-all shadow-lg shadow-amber-500/5 group"
                >
            ...
            <div className="theme-card border border-black/5 rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-8 relative z-10">
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-40 mb-2">Weekly Revenue Matrix</p>
                    ...
                    <div className="bg-black/5 dark:bg-white/5 p-4 rounded-2xl border border-black/5">
                        <p className="text-[9px] font-black uppercase tracking-widest opacity-30 mb-1">Unit Profitability Index</p>
                        <p className="text-xl font-black text-amber-600 dark:text-amber-400 tracking-tight">₹{avgOrderValue.toFixed(2)}</p>
                    </div>
                </div>
                ...
                {(!data.dailySales || data.dailySales.length === 0 || data.dailySales.every(d => d.sales === 0)) ? (
                    <div className="h-64 flex flex-col items-center justify-center theme-card-item rounded-3xl border border-black/5 border-dashed relative z-10">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-40">Static Activity Spectrum</p>
                    </div>
                ) : (
                            ...
                            <div key={idx} className="flex-1 flex flex-col items-center gap-4 group">
                                <div className="relative w-full flex flex-col justify-end h-48">
                                    <motion.div
                                        ...
                                        className="bg-gradient-to-t from-amber-600 to-amber-400 rounded-2xl group-hover:from-amber-500 group-hover:to-purple-500 transition-all relative min-h-[6px] shadow-lg shadow-amber-500/10"
                                    >
            ...
            {data.restaurantStats && data.restaurantStats.length > 0 && (
                <div className="theme-card border border-black/5 rounded-[2rem] overflow-hidden shadow-2xl">
                    <div className="p-8 border-b border-black/5 bg-amber-500/5">
                        <h3 className="text-xl font-black uppercase tracking-tighter">Establishment Intelligence Breakdown</h3>
                    </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 bg-black/5 dark:bg-white/5">
                                    ...
                                    <th className="px-8 py-5 text-right">Live Flow</th>
                                    <th className="px-8 py-5 text-right">Gross Output</th>
                                </tr>
                            </thead>
